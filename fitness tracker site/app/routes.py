...
def save_progress():
    ...
    existing_activity = next(
        (activity for activity in user_activities if activity['name']
         == name and activity['category'] == category),
        None)
    ...
    if existing_activity:
        ...
        user_activities.append(
            {'name': name, 'category': category, 'progress': progress})
...
def add_activity():
    ...
    activities_data[email].append(
        {'name': name, 'category': category, 'progress': 0})
...
def manage_goals():
    ...
    if not email or email not in users:
        return jsonify(
            {"message": "User not logged in or data not found."}), 404
...
def contact():
    if request.method == 'POST':
        ...
        return redirect(url_for('bp.contact'))  # Redirect after POST to show flash messages on GET
...
def get_chart_data():
    # Default to 'weekly' if no period is specified
...
def get_recent_activities():
    ...
    if period == 'weekly':
        activities = [
            {"date": "2024-12-01", "type": "Running",
                "duration": "30 mins", "calories": 300},
            {"date": "2024-12-03", "type": "Cycling",
                "duration": "45 mins", "calories": 500},
            {"date": "2024-12-05", "type": "Yoga",
                "duration": "60 mins", "calories": 200}
        ]
    elif period == 'monthly':
        activities = [
            {"date": "2024-11-01", "type": "Running",
                "duration": "180 mins", "calories": 1300},
            {"date": "2024-11-07", "type": "Cycling",
                "duration": "250 mins", "calories": 1500},
            {"date": "2024-11-14", "type": "Yoga",
                "duration": "280 mins", "calories": 2000},
            {"date": "2024-11-21", "type": "Gym",
                "duration": "300 mins", "calories": 3000}
        ]
    elif period == 'yearly':
        activities = [
            {"date": "2024-10-01", "type": "Running",
                "duration": "2000 mins", "calories": 13000},
            {"date": "2024-11-01", "type": "Cycling",
                "duration": "2700 mins", "calories": 15000},
            {"date": "2024-12-01", "type": "Yoga",
                "duration": "5000 mins", "calories": 20000}
        ]
    else:
        activities = {
            "error": "Invalid period. Please use 'weekly', 'monthly', or 'yearly'."}
...
def signup():
    ...
    if email in users or any(
            user['username'] == username for user in users.values()):
        # Pass a message back to the signup template
        return render_template(
            'signup.html', message="Email or username already exists.")
    else:
...
# Route for login page (GET) and handling login (POST)
...
def login():
    ...
    ) if user['email'] == identifier or user['username'] == identifier), None)

    if user and check_password_hash(user['password'], password):
        # Generate auth_token (using a simple UUID for this example)
        auth_token = str(uuid4())

        # Set cookies for email, username, and auth_token
        response = make_response(render_template('home.html'))
        response.set_cookie('email', user['email'])
        response.set_cookie('username', user['username'])
        response.set_cookie('auth_token', auth_token)  # Set auth_token
...
@bp.route('/check_login_status', methods=['GET'])
...
def logout():
    response = make_response(redirect(url_for('bp.login')))
...
def update_data():
    ...
    if email in users:
        ...
        user = next(
            (u for u in users.values() if u['username'] == username),
            None)
    ...
    if new_password:
        # Update password for the current email
        users[email]['password'] = new_password

    if new_username:
        # Update username for the current email
    ...
    response.set_cookie(
        ...
        path='/')  # Set email cookie
    response.set_cookie(
        'username',
        new_username or users[email]['username'],
        max_age=60 * 60 * 24 * 365,
        path='/')  # Set username cookie
...
def delete_data():
    ...
    if user:
        ...
        if email:
            ...
            del users[next((k for k, v in users.items()
                           if v['username'] == username), None)]
